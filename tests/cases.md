...
这套用例检查的不是接口“能不能返回”，而是真实模型是否理解团播复活场里的主播、主持和用户博弈。部署前必须跑完 **8 类、10 次请求**；任一机器硬断言失败，或人工检查出现关键误读，都先迭代 `worker/prompt.js`，再全量重跑。
...
发布门槛：10/10 机器通过，案例 1 两轮均 passed，并且案例 4、5、8 的人工语义检查通过。
...
4. “你上几张”是在给大哥决定权，不能误判成命令；“求求、可怜我”才是把自己放低。
