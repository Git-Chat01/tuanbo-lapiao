...
5. 给出用户此刻能执行的要票动作；不要求固定票差、指定数量或特定人名
...
复盘页采用轻量“话术闯关”：每次明确显示当前唯一关卡、已经做到什么、只差什么和一个可执行的解题方法。主播始终在自己的原话上修改，未改动时不能原样重复提交；同一关试到第二次仍未通过，系统会先消除自责，再自动展开与当前场景和关卡一致的两个小动作，避免一直盲改。核心逻辑成立但仍有非核心项可补时，页面会明确写“核心逻辑已过关”，不会伪装成固定 5/5。
...
`passed` 是后端硬门槛：`user_reason` 与 `vote_instruction` 必须为 `met` 且有非空证据，`line_reviews.original` 按顺序覆盖完整原稿、逐句没有 `wrong`，安全字段与 `round_dynamics` 契约完整，并且没有显性低姿态、persona/AI 味或任何红线。模型给 `almost` 但硬条件全部满足时会被确定性晋级；模型明确给 `off` 时不会被两个状态位洗掉。红线始终一票否决。
