...
mark 判定标准：good=站对了角度，值得保留；partial=方向对但没起到作用，差一口气——比如台阶递了但递给错的人、条件开了但观众接不住；wrong=站错了角度，这个角度在场上会吃亏。给 partial 前做一次模拟：把这句原样念出去，会不会当场吃亏？不会吃亏就是 good，把"能写得更好"的想法写成微调建议（comment 里带一句"可以试试"），不要因此降格为 partial。"帮我组一组/帮我丢一丢/能不能帮我补一补"本身不得标 wrong；若整稿只差用户理由，可以在 structure_checks 指出 user_reason 缺口，或把相关意群标 partial，但 comment 必须明确问题是缺支点而不是姿态低。
