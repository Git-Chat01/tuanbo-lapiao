...
- 先区分话的用途：陈述“你已经认了”才是在宣称承诺；邀请、提议、反问是在给对方选择下一步，不能被听成宣布对方已经答应。请求不必机械带“愿意/方便”才有选择空间。判 misread 必须指出原话实际断言了什么、与哪条已知事实矛盾；不能把你猜出的恶意、债务或强迫当成原话的断言。观众刚上了下去票又问“医药费怎么组”时，“你先认一个？”是沿双方玩笑递一个可选择的组法，本身已给出一个位置；不能因没先念玩法说明就判没回答。相反，观众明确说“我想帮，现在怎么做”时，主播若只回问“你方便吗”而不给认位、等口令等可执行步骤，才是没接住主动求助。结合问话用途判断，不把“怎么”字当成统一的说明书要求。
...
3. 风险单独判断：明确求求你/可怜我/跪下等乞求自贬、逼迫消费、阶段错位是问题；帮我组组队、方便吗、身后没人不是自动卑微。不要编造用户喜好、承诺、到账、规则和结果。跨两处意群重复泛夸—求帮—口号收尾，且没有针对新反馈改变互动，才考虑 persona，引用至少两处说明共同机制；一处玩笑、自然完整句不是 AI 味。不要只靠“英雄救美”“人多力量大”任一个词触发标签，检查整篇是否让观众有真实回应空间。
   同时看内容和频次：若已知这轮长期没有实质表演或互动，只密集重复“组一组”“就差最后一点”催礼、制造紧迫感，即使差额真实也判 off。理由要说出“长时间无实质内容＋反复催礼”的组合问题；修改的第一步是收住催礼，先恢复表演或真实互动，再视观众回应决定是否邀请，不能只建议点名补位或把缺口说得更精确。若只是表演后一次按真实进度邀请，则不能按这些词单独判违规或模板腔。没有整场信息时只评当前稿的重复与已知事实，不编造场外违规。观众明确说预算到顶、只想看时，不能继续点名催他出钱；可给他观看或免费回应的位置，再找其他愿意参与者。
...
interaction_review 是必填的公开判断摘要，signal_refs 最多6项，只能引用实际存在的 scenario 字段名（例如 userSignal、phase、openRemaining）或 timeline:从0开始的编号、script:从0开始的编号；禁止编位置或写原文代替位置。script_refs 必须使用本次输入 segments 的从0开始的实际下标，只能选 requiredSegmentIndexes 中存在的数字，不能空、重复或写大于末位的编号；misread 时只选发生误读的句子。没有外部现场时可用 script:编号，但须说明只是原稿陈述、未验证用户反馈。aligned 表示与已知事实相符（不代表必过关）；misread 必须有明确矛盾并把对应原句标 wrong；uncertain 表示真实关系待确认，不能说成用户肯定喜欢或不喜欢。
